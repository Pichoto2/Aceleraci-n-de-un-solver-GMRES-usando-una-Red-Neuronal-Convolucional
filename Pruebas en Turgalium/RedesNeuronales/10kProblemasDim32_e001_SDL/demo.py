...
from src_dir import cnn_preconditionerOnline_timed_2D, CNNPredictorOnline_2D,\
                    timer, GMRES, PreconditionerTrainer, CNN_30, FluidNet2D10, FluidNet2D30, SingleDenseLayer
import sys
import os
import shutil
...
if len(sys.argv) > 1:
    ...
    os.mkdir(os.path.join(nombre_carpeta, "curvasAprendizaje"))
...
dim = 32 # Dimensión de las matrices b y x0
...
nn_precon = CNNPredictorOnline_2D(InputDim, OutputDim, Area, dx, SingleDenseLayer) # intialize the NN
...
print("Número de parámetros: ", sum(p.numel() for p in nn_precon.model.parameters() if p.requires_grad)) # Cambiado **
print("Número de sets de entrenamiento: ", np.shape(nn_precon.x)[0]) # Cambiado **
...
np.savetxt(os.path.join(nombre_carpeta, "SpeedUp_Total.txt"),Ratio)
pp.clf()
