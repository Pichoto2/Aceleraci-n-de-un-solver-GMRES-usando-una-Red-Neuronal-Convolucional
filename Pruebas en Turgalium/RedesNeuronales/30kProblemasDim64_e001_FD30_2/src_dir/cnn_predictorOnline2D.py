...
class CNNPredictorOnline_2D(object):
	...
	# Cambiado
        if self.trainCount == 0 or self.trainCount % 20 == 0:
            epochX = np.arange(0,epoch)
            pp.semilogy(epochX, self.loss_val[2::])
            pp.savefig(os.path.join(sys.argv[1], "curvasAprendizaje", "curvaAprendizaje_"+str(self.trainCount)+".png"))
            pp.clf()

            np.savetxt(os.path.join(sys.argv[1], "curvasAprendizaje", "curvaAprendizaje_"+str(self.trainCount)+".txt"),self.loss_val[2::]) 
