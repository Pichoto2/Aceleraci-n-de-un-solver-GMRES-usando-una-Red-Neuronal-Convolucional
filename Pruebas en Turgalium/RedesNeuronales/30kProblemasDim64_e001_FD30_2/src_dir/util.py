...
def resid(A, *args, **kwargs):
    '''
    Function to calculate the residue of the given problem
    - A: matrix or laplace operator
    '''
    if isinstance(A, np.matrix): 
        A_op = lambda x: matmul_a(A, x) # Transform matrix to operator -> A*x
...
def resid_kernel(A, x, b):
    '''
    Auxiliar function to calculate the residue of the given problem
    - A: Laplace operator
    - x: GMRES solution (matrix)
...
def timer(func):
    ...
    def wrapper_timer(*args, **kwargs):
        ...
        value = func(*args, **kwargs) # do the function
        end_time = time.perf_counter()  # finished time
    ...
    return wrapper_timer # return the func value and its runtime


def prob_norm(b):
    '''
    Calculate the norm of the matrix b and its normalized maximum
    '''
    # Compute 2-norm of RHS(for scaling RHS input to network)
    b_flat     = np.reshape(b, (1, -1), order='F').squeeze(0) # convert matrix to vector
...
def moving_average(a, n):
    '''
    Function that returns the moving avarage of the list a with the window number of elements
    '''

    # modify window if n < 25 else do the avg with the 25 last elements
    if n == 0:
        return 0
    elif n < 1:
