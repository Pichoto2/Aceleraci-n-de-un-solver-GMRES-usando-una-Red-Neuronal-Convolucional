...
from src_dir import cnn_preconditionerOnline_timed_2D, CNNPredictorOnline_2D,\
                    timer, GMRES, PreconditionerTrainer, CNN_30, FluidNet2D10
import sys
import os
import shutil
...
dim = 30 # Dimensión de las matrices b y x0
...
n = int(n_steps/10)
RedRatio = [Ratio[i:i + n] for i in range(0, len(Ratio), n)]

pp.plot(RedRatio[0])
pp.xlabel('i = 1-'+str(n_steps)+'   |   cada '+str(int(n_steps/n)))
pp.xticks([])
pp.ylabel('GMRES/MLGMRES')
