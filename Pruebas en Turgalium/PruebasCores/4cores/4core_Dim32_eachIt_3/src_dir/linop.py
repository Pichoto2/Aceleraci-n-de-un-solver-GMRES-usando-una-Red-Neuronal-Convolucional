...
def appl_2d(op, x, Nx, Ny):
    '''
    Apply the function op to the matrix x of Nx x Ny
    '''
    Ax = np.zeros((Nx, Ny))
...
def mk_laplace_2d(Nx, Ny, bc="dirichlet", xlo=0, xhi=0, ylo=0, yhi=0):
    ...
    - Nx: number of points in the x-axis grid
    - Ny: number of points in the y-axis grid
    - bc: type of boundary condition, the options can be dirichlet or periodic
    - xlo: left dirichlet condition 
    - xhi: right dirichlet condition
    - ylo: bottom dirichlet condition
    - yhi: top dirichlet condition

    return the function appl_2d with the corresponding size and boundary condition
    '''
    def build_bc_1(x_in):
        '''
        Apply the dirichlet condition to the matrix x_in of Nx x Ny

        return the tensor Nx+2 x Ny+2 with the boundary conditions applied
        '''
        x_out = np.zeros((Nx + 2, Ny + 2)) # initialize return matrix

        x_out[1:Nx+1, 1:Ny+1] = x_in[:, :] # same inside

        x_out[0,    :] = xlo # left boundary condition
        x_out[Nx+1, :] = xhi # right boundary condition
        x_out[:,    0] = ylo # bottom boundary condition
        x_out[:, Ny+1] = yhi # top boundary condition
    ...
    def build_bc_2(x_in):
        '''
        Apply the periodic condition to the matrix x_in of Nx x Ny
        ...
        x_out[0,    1:Nx+1] = x_in[-1, :] # left boundary condition
        x_out[Nx+1, 1:Nx+1] = x_in[0,  :] # right boundary condition
        x_out[1:Nx+1,    0] = x_in[:, -1] # bottom boundary condition
        x_out[1:Nx+1, Ny+1] = x_in[:,  0] # top boundary condition
    ...
    def laplace_2d(x, i, j):
        '''
        Apply laplace inside the domain to x in the position i,j
        '''
        return (-4*x[i, j] + x[i-1, j] + x[i+1, j] + x[i, j-1] + x[i, j+1])

    # Select the bc operator
    if bc == "dirichlet": 
...
def appl_2d_Tensor(op, x, Nx, Ny):
    '''
    Apply the function op to the tensor x of Num matrixes of Nx x Ny using GPU
    '''
    Num=x.size(0)
...
def mk_laplace_2d_Tensor(Nx, Ny,bc="dirichlet", xlo=0, xhi=0, ylo=0, yhi=0):
    ...
    return the function appl_2d_Tensor with the corresponding size and boundary condition
    '''
    def build_bc_1_Tensor(x_in):
        '''
        Apply the dirichlet condition to the tensor x_in of Num matrixes of Nx x Ny using GPU

        return the tensor Num x Nx+2 x Ny+2 with the boundary conditions applied
        '''
        Num=x_in.size(0) # Num of matrixes 
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") # GPU
        x_out = torch.zeros(Num,Nx + 2, Ny + 2,requires_grad=False).to(device) # initialize the return tensor
    ...
    def build_bc_2_Tensor(x_in):
        '''
        Apply the periodic condition to the tensor x_in of Num matrixes of Nx x Ny using GPU
