...
class FluidNet2D10(torch.nn.Module):
    ...
    def __init__(self, D_in, D_out):
        ...
        # define the functions to put the input tensor a 0s around the matrices
        ...
        # convert the matrices of the input tensor to the desired size bilinearly
    ...
    def forward(self, x,DataSetSize,Factor):
        '''
        Function forward propagation of the neural network with 2 extra resblock
        - x: tensor with the scalated b 
        - DataSetSize: number of matrices in the tensor 
        - Factor: factor with how b was scalated (maximum b) 
        return the prediction of the x
        '''
        ...
        if (DataSetSize < Factor*1):
            z = self.ConvInit1(self.pad_7Kernel(x2)) # start -> ConvInit1
            z1 = self.relu(self.ConvInit2(self.pad_5Kernel(z))) # ConvInit1 -> ConvInit2
            z2 = self.AVG(z1) # Avg of result of ConvInit1
            z3 = self.AVG(z2) # Avg of result of ConvInit2
            # Full scale
            z1 = self.relu(self.Conv11(self.pad_5Kernel(z1))) # ConvInit2 -> Conv11
            z1 = self.relu(self.Conv12(self.pad_5Kernel(z1))) # Conv11 -> Conv12
            # Downsample1 scale
            z2 = self.relu(self.Conv21(self.pad_3Kernel(z2))) # Avg ConvInit1 -> Conv21
            z2 = self.relu(self.Conv22(self.pad_3Kernel(z2))) # Conv21 -> Conv22
            z2 = self.Upsample(z2) # Upsample to dim x dim
            # Downsample2 scale
            z3 = self.relu(self.Conv31(z3)) # Avg ConvInit2-> Conv31
            z3 = self.relu(self.Conv32(z3)) # Conv31 -> Conv32
            z3 = self.Upsample(z3) # Upsumple to dim x dim
            # Sum all convolution output scales
            z = z1 + z2 + z3
            z = self.relu(self.Conv4(z)) # Conv12 + Conv22 + Conv32 -> Conv4
            z = self.relu(self.Conv5(z)) # Conv4 -> Conv 5
            z = self.ConvOut(z) # Conv5 -> ConvOut
            return z.squeeze(1)
        ...
        if (DataSetSize >= Factor*1):
            ...
            #resblock 1
            y = self.relu(z) 
            y = self.relu(self.Conv_Post1(self.pad_3Kernel(z))) # Conv12 + Conv22 + Conv32 -> Conv_Post1
            y = self.Conv_Post2(self.pad_3Kernel(y)) # Conv_Post1 -> Conv_Post2
            z = z + y 
            z = self.relu(z)
            z = self.relu(self.Conv4(z)) # Conv12 + Conv22 + Conv32 + Conv_post2 -> Conv4
            z = self.relu(self.Conv5(z)) # Conv4 -> Conv5
            z = self.ConvOut(z) # Conv5 -> ConvOut
            return z.squeeze(1)
        ...
        if (DataSetSize >= Factor*2):
            ...
            z3 = self.relu(self.Conv31(z3)) # Avg ConvInit2-> Conv31
            z3 = self.relu(self.Conv32(z3)) # Conv31 -> Conv32
            z3 = self.Upsample(z3) # Upsample to dim x dim
            # Sum all convolution output scales
            z = z1 + z2 + z3
            #resblock 1
            y = self.relu(z) 
            ...
            #resblock 2
            y = self.relu(z) 
            y = self.relu(self.Conv_Post3(self.pad_3Kernel(z))) # Conv12 + Conv22 + Conv32 + Conv_Post2 -> Conv_Post3
            y = self.Conv_Post4(self.pad_3Kernel(y)) # Conv_Post3 -> Conv_Post4
            z = z + y
            z = self.relu(z)
            z = self.relu(self.Conv4(z)) # Conv12 + Conv22 + Conv32 + Conv_Post2 + Conv_Post4 -> Conv4
            z = self.relu(self.Conv5(z)) # Conv4 -> Conv5
            z = self.ConvOut(z) # Conv5 -> Conv4
            return z.squeeze(1)
